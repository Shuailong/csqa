...
# @Date: 2019-05-18 17:02:11
# @Last Modified by: Shuailong
# @Last Modified time: 2019-05-18 17:22:58
...
@Model.register("csqa-bert-attn")
class CSQABertAttn(Model):
    """
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        check_dimensions_match(bert.get_output_dim() * 3, classifier.get_input_dim(),
    ...
    def forward(self,  # type: ignore
        ...
        evidence_hidden = evidence_hidden.view(
            batch_size, 5, evi_num, emb_size)
        # evidence_hidden: batch, 5, evi_num, emb
        # scores: batch, 5, evi_num
        evidence_summary = weighted_sum(
            evidence_hidden, scores)  # batch, 5, emb

        question_hidden = question_hidden.unsqueeze(
            1).expand(batch_size, 5, emb_size)
        cls_hidden = torch.cat([question_hidden, choice_hidden, evidence_summary],
                               dim=-1)
