...
Adapted to allow more token types.
...
class BertEmbedder(TokenEmbedder):
    ...
    def forward(self,
        ...
        if needs_split:
            ...
            unpacked_embeddings = torch.split(
                all_encoder_layers, batch_size, dim=1)
...
@TokenEmbedder.register("bert-pretrained-sl")
class PretrainedBertEmbedder(BertEmbedder):
    ...
    def __init__(self,
                 ...
                 top_layer_only: bool = False,
                 type_vocab_size: int = 2) -> None:
        model = PretrainedBertModel.load(pretrained_model)
        if type_vocab_size > 2:
            print('Type vocab size > 2, expand type vocab embeddings.')
            expand_embs = torch.nn.Embedding(
                type_vocab_size, model.config.hidden_size)
            expand_embs.weight.data.normal_(
                mean=0.0, std=model.config.initializer_range)
            pretrained_type_vocab_embeddings = model.embeddings.token_type_embeddings.weight
            expand_embs.weight[:model.config.type_vocab_size,
                               :] = pretrained_type_vocab_embeddings
            model.embeddings.token_type_embeddings.weight.data = expand_embs.weight.data
